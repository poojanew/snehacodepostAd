"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var ProductService_1 = require("../../services/ProductService/ProductService");
var router_1 = require("@angular/router");
var AdvertisementTableComponent = (function () {
    function AdvertisementTableComponent(productService, router) {
        this.productService = productService;
        this.router = router;
    }
    AdvertisementTableComponent.prototype.displayAd = function () {
        return this.productService.getAllAdvertises();
    };
    AdvertisementTableComponent.prototype.deleteObject = function (index) {
        return this.productService.deleteAdvertise(index);
    };
    AdvertisementTableComponent.prototype.editObject = function (index) {
        this.router.navigate(['/EditProduct']);
    };
    return AdvertisementTableComponent;
}());
AdvertisementTableComponent = __decorate([
    core_1.Component({
        selector: 'advertisementTable',
        templateUrl: "./advertisementtable.html",
    }),
    __metadata("design:paramtypes", [ProductService_1.ProductService, router_1.Router])
], AdvertisementTableComponent);
exports.AdvertisementTableComponent = AdvertisementTableComponent;
//# sourceMappingURL=AdvertisementTableComponent.js.map